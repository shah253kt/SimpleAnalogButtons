...
#define ANALOG_BUTTON_MAX_COUNT 8
#endif
...
#define ANALOG_BUTTON_DEFAULT_MIN_VALID_VALUE 300
#endif
...
public:
    ...
    SimpleAnalogButtons(uint8_t pin, WiringPinMode mode = INPUT, uint16_t minValidValue = ANALOG_BUTTON_DEFAULT_MIN_VALID_VALUE, uint16_t samplingInterval = ANALOG_BUTTON_DEFAULT_SAMPLING_INTERVAL);
#else
    SimpleAnalogButtons(uint8_t pin, uint8_t mode = INPUT, uint16_t minValidValue = ANALOG_BUTTON_DEFAULT_MIN_VALID_VALUE, uint16_t samplingInterval = ANALOG_BUTTON_DEFAULT_SAMPLING_INTERVAL);
#endif

    ~SimpleAnalogButtons();

    void addButton(uint16_t minMean, uint16_t maxMean);
